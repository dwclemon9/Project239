import type { Intensity, Session, WeekSummary, Wellness } from './types';
...
/** Bucket sessions into Monday weeks, including empty weeks so charts keep a true x-axis. */
export function weeklySummaries(sessions: Session[], weeks: number, asOf: string): WeekSummary[] {
  ...
  for (let i = weeks - 1; i >= 0; i--) {
    ...
    buckets.set(ws, {
      weekStart: ws, distance_m: 0, easy_m: 0, moderate_m: 0, hard_m: 0,
      duration_sec: 0, sessions: 0, load: 0,
    });
  }

  for (const s of sessions) {
    const bucket = buckets.get(weekStart(s.date));
    if (!bucket) continue;
...
export function easyShare(weeks: WeekSummary[]): number | null {
  const total = weeks.reduce((sum, w) => sum + w.distance_m, 0);
  if (total <= 0) return null;
  return weeks.reduce((sum, w) => sum + w.easy_m, 0) / total;
}
